...
Meteor.startup(function() {
	Session.set("return", {type: "conversions"});
	Session.set("sort_order", {conversions: -1});
	Session.set("sorting", "conversions_desc");

	var offers_url = "http://mmotm.api.hasoffers.com/Api/json?Format=json&Target=Report&Method=getStats&Service=HasOffers&Version=2&NetworkId=mmotm&NetworkToken=NETjE4MoLg7NarETCDruHecVmgLHbN&fields%5B0%5D=Stat.clicks&fields%5B1%5D=Stat.conversions&fields%5B2%5D=Stat.currency&fields%5B3%5D=Stat.payout&groups%5B0%5D=Offer.name&totals=1&limit=1000000";
	if(games.find().count() === 0){
		...
		for (var key in jsondecoded.response.data.data) {
			if (jsondecoded.response.data.data.hasOwnProperty(key)) {
				...
				if(games.find({name:name}).count() === 0 ){
					games.insert({clicks: clicks, conversions: conversions, payout: payout, currency: currency, name: name});
					console.log("Inserted data for " + name);
				}else{
					var id = games.findOne({name:name})._id;
					var newClicks = parseInt(games.findOne({name:name}).clicks) + clicks;
					var newConversions = parseInt(games.findOne({name:name}).conversions) + conversions;
					var newPayout = parseInt(games.findOne({name:name}).payout) + payout;
					games.update({_id:id},{$set: {clicks:newClicks,conversions:newConversions,payout:newPayout}});
